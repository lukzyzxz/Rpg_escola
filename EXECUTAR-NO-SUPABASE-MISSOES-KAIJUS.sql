-- ============================================================================
-- NAVE 3B — MISSÕES, ATRIBUTOS, KAIJUS E PEÇAS DO MECHA
...
-- Restaura/recalcula todo mundo. Sem missões marcadas: 20 / 0 / 5 / 0.
do $$
declare
    v_usuario record;
begin
    for v_usuario in select id from auth.users loop
...
alter table public.mecha_kaijus_catalogo
    ...
    add column if not exists status text not null default 'CATALOGADO';

insert into public.mecha_kaijus_catalogo
    (id, nome, ordem, imagem_path, descricao, status)
values
    ('rei-porco', 'Kaiju Porco', 1, 'assets/kaijus/kaiju-porco.jpg',
     'Criatura de impacto bruto e grande resistência. Seus materiais favorecem vida e mobilidade.', 'CATALOGADO'),
    ('rei-verdejante', 'Rei Verdejante', 2, 'assets/kaijus/rei-verdejante.png',
     'Soberano biológico da floresta, capaz de transformar crescimento e níveis de combate em poder.', 'CATALOGADO'),
    ('cobra-falante', 'Cobra Falante', 3, 'assets/kaijus/cobra-falante.png',
     'Predadora estratégica cujas peças convertem agilidade e treinamento em ataque e vida.', 'CATALOGADO'),
    ('hidra', 'Hidra Caótica', 4, 'assets/kaijus/hidra-caotica.png',
     'Entidade multicéfala de regras imprevisíveis, focada em repetição de cartas e vida extrema.', 'CATALOGADO'),
    ('tartaruga-dragao', 'Tartaruga Dragão', 5, 'assets/kaijus/tartaruga-dragao.png',
     'Fortaleza dracônica que troca vida por redução de dano e escala com todos os níveis do piloto.', 'CATALOGADO')
on conflict (id) do update set
    nome = excluded.nome,
    ordem = excluded.ordem,
